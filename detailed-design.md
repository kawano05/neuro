...
#### 9.3.3 書き出す時刻は日本時間（2026-08-28）

保存はUTCのまま。**書き出しだけ**を日本時間へ直す（`src/lib/utils.js` の
`toJstIso`、固定 +09:00。`Asia/Tokyo` は夏時間を持たないので固定で厳密）。

理由: 日本時間の夕方に測った回はUTCでは同じ日の朝、深夜の回は前日になる。
CSVを「日ごと」に集計すると境界がずれたまま数が出る——数字は出るので気づかない。

- 列名も変える（`startedAtIso` → `startedAtJst`、`endedAtIso` → `endedAtJst`、
  ログの `time` → `time_jst`、効果測定CSVの4列に `_jst` を付ける）。中身の意味を
  変えるのに名前を残すと、以前の書き出しをUTCとして読んでいる手元の集計が、
  黙って9時間ずれた値を受け取る。名前を変えれば、そこで止まって気づける。
- オフセット（`+09:00`）は必ず文字列に残す。落とすとUTCの値と見分けがつかない。
- 端末のタイムゾーン設定には依存させない。iPadの設定が違っていても書き出しは
  常に日本時間になる。
- 生データJSONは state をそのまま入れる（保存はUTCのまま）。読む人のために
  `exportedAtJst` だけ併記する。
