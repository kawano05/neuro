...
export const cranePresets = {
   ...
   * 「grip 圏の半径 × sweepMs/100」。既定では各軸 ±121ms になる。
   *
   * 経緯: もとは 2400ms / 12（±144ms）。狙いの手がかり——床に描いた走査線、
   * 掴める範囲のリング、目標を通過したときの音——を入れたことで素の難度が
   * 下がったので 2000ms / 10（±100ms）まで詰めたが、実際に遊ぶときつすぎた。
   * その中間に置いている。
   *
   * 走査を速くするのと許容を狭くするのは要求精度としては等価だが、速くすると
   * 視覚追従そのものが辛くなり、訓練したい「狙って押す」以外のところで詰まる。
   * だから許容側を主に詰める。速度を 2400 から下げているのは難度というより、
   ...
   *
   * 既定では toleranceR が 11 → 14.85 → 18.7 と広がり、要求精度は各軸
   * ±121ms → ±163ms → ±206ms。狙って押すこと自体が訓練の対象である
