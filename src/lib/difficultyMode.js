 ...
 * エンドレスの難度の上げ方の版。
 *
 * 傾斜の定数（何試行ごとに何%か、下限をどこに置くか）は、そのままその回の
 * プロトコル。ところが記録に残っていたのは `endless: true` だけだったので、
 * 定数を変えると変更前後の回が見分けられなくなる——実際に 2026-08-29 に
 * 下限の当て方を直しており、その前後で難度の上がり方が違う。
 *
 * 版を上げるのは「同じ版どうしなら比較してよい」という約束を壊すときだけ。
 * 見た目や文言だけを直したときは上げない。
 *
 *   endless-v1 … 初版。下限を Math.max だけで当てていたため、下限より
 *                厳しい設定から始めた回が易化していた（欠陥あり）。
 *   endless-v2 … 始めた条件より易しくしないことを保証（2026-08-29）。
 */
export const ENDLESS_PROTOCOL_VERSION = "endless-v2";
